...
const SignUp = () => {
  ...
  const handlesubmit = async (e) => {
    ...
    const response = await fetch(`${host}/api/auto/createuser`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-black">
      <div className="bg-[#1e1e1e] p-8 rounded-[20px] shadow-2xl w-full max-w-sm animate-fade-in border border-zinc-800">
        <h1 className="text-2xl font-bold text-center text-white mb-6">Create a New Account</h1>
        <form onSubmit={handlesubmit} className="flex flex-col gap-4">

          <div className="flex flex-col">
            <label className="text-gray-400 mb-1 text-sm">Full Name *</label>
            <input
              required
              name="name"
              value={cred.name}
